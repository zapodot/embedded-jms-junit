...
public interface SendReceivable {
    default String sendReceive(final ConnectionFactory connectionFactory,
                               final String queueName,
                               final String textMessage) throws JMSException {
        final ExecutorService executorService = Executors.newFixedThreadPool(2);
        try {
            final Future<String> messageFuture = executorService.submit(messagesFromTestQueue(connectionFactory,
                                                                                              queueName));
